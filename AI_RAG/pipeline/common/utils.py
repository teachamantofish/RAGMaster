...
def get_csv_to_process(crawl_id=None) -> Dict[str, Any]:
    """
    Return a dict with:
      - 'input_csv_row': the input row from metadataconfig.csv
      - 'cwd': Path object for the working directory
    Uses CRAWL_ID if no crawl_id is provided.
    Raises ValueError if crawl_id not found.
    """
    if crawl_id is None:
        crawl_id = _CRAWL_ID
    key = str(crawl_id)
    with open(_CSV_PATH, newline='', encoding='utf-8') as f:
        ...
        for r in reader:
            if r['ID'] == key:
                base_dir = r.get('BASE_DIR') or 'run'
                cwd = Path(_ROOT_DIR) / base_dir
                return {
                    'input_csv_row': r,
                    'cwd': cwd
                }
    raise ValueError(f"Crawl ID {crawl_id} not found in metadataconfig.csv")
