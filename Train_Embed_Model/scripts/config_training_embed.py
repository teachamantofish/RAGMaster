...
TRAINING_DATA_DIR = BASE_CWD / EMBED_TRAINING_SUBDIR
...
OUTPUT_MODEL_PATH = BASE_CWD / EMBED_OUTPUT_SUBDIR # PEFT adapter weights and other trainer artifacts
TOKENIZED_DATA_DIR = TRAINING_DATA_DIR / TOKENIZED_SUBDIR # Tokenized dataset location (created by scripts/tokenize_triplets.py)

EMBED_INSTRUCTION = "Given a FrameMaker MIF or JSX scripting question, retrieve information about MIF object structure and JSX script behavior."
MIN_CONTENT_LENGTH = 50  # Minimum characters for content to be useful for training
MAX_CONTENT_LENGTH = 5000  # Maximum characters to avoid overwhelming the model
MIN_TRIPLETS_PER_CATEGORY = 300  # Minimum triplets per category/domain
...
EASY_TRIPLET_RATIO = 0.25
MEDIUM_TRIPLET_RATIO = 0.35
HARD_TRIPLET_RATIO = 0.40

# Triplet filtering: discard triplets where the annchor-negative similarity is too high
EASY_NEGATIVE_MAX_COSINE = 0.65     # Max similarity for negatives compared to anchor
MEDIUM_NEGATIVE_MIN_COSINE = 0      # None for now as this shrinks the hard pool too much
MEDIUM_NEGATIVE_MAX_COSINE = 1.0   # Max similarity for negatives compared to anchor
HARD_NEGATIVE_MAX_COSINE = 1.0     # Max similarity for negatives compared to anchor
...
TRAINING_CONFIG = {
    "epochs": 2,  # Number of training passes Stop if loss < ~0.05 AND first batches hit 0.0
    "learning_rate": 7e-6,  # Higher LR for faster convergence on smaller model-lower LR for stabler pace
    ...
    # If the model is large, set to True and load in 8-bit with device_map='auto'. Requires bitsandbytes & accelerate.
    ...
    "triplet_margin": 1,  # Margin for triplet loss (distance between positive and negative). Softer margin improves smoothness
...
OLLAMA_EXPORT = {
    ...
    "context_length": 32768,  # Context window for the model
...
VECTOR_DB_CONFIG = {
    "embedding_dimension": 1024,  # Your preferred dimension
