...
def setup_global_logger(
    ...
    enforce_standard_headers: bool = True,
    ...
    log_filename = f"a_{base_name}{log_suffix}"
    log_path = cwd / log_filename  # Use Path for consistency
    
    # Validate headers: caller must supply headers (list/tuple). When
    # enforce_standard_headers=True, the first three columns must be
    # Date/Level/Msg for compatibility with historical logs.
    if headers is None:
        raise ValueError("setup_global_logger requires a 'headers' argument")
    if not isinstance(headers, (list, tuple)):
        raise ValueError("headers must be a list or tuple of column names")
    if enforce_standard_headers:
        if len(headers) < 3:
        ...
        if not (first_three[0] == 'date' and first_three[1] == 'level' and first_three[2] in ('msg', 'message')):
            raise ValueError("headers must start with ['Date','Level','Msg'|'Message'] in that order when enforce_standard_headers=True")
    else:
        if len(headers) < 1:
            raise ValueError("At least one header column is required when enforce_standard_headers=False")

    # CSV-safe formatter that always emits columns in the declared order.
    class CSVFormatter(logging.Formatter):
        def __init__(self, columns, datefmt=None, enforce_standard=True):
            super().__init__()
            self.columns = [str(c) for c in columns]
            self.datefmt = datefmt or '%m/%d'
            self.enforce_standard = enforce_standard
    ...
    try:
        ...
        file_handler.setFormatter(CSVFormatter(headers, datefmt='%m-%d %H:%M', enforce_standard=enforce_standard_headers))
    ...
    # Log initialization using the CSV formatter (will produce a row) if desired
